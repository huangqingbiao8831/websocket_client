 ...
 *       2,bin
 **************************** */
int libwsclient_send(wsclient *client, char *strdata,int len,int type)  {
	wsclient_error *err = NULL;
	struct timeval tv;
	unsigned char mask[4];
	unsigned int mask_int;
	...
	finNopcode = 0x80 + (unsigned char)type;
        printf("finNopcode = %02x,payload_len = %d \n",finNopcode,payload_len);
	if(payload_len <= 125) {
		frame_size = 6 + payload_len;
		payload_len_small = payload_len;
	...
	*(data+1) = payload_len_small | 0x80; //payload length with mask bit on
	...
	printf("send payload_len = %d \n",payload_len);
	memcpy(data+payload_offset, strdata, payload_len);
